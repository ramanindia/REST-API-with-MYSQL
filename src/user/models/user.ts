...
export const userModel = {
  ...
  registerUser: async (requestData: any) => {
    ...
    const status = requestData.status ?? 1;
     ...
     var insertValue = [ uuidv4(),roleID,timezoneID,signupType, name,email,countryID,mobileNo,address,
      userPassword, password, 1,1,status];
    var insertQuery =
      "insert into users set userUUID=?,roleID=?,timezoneID=?,signupType=?,name=?," +
